...
def test_intake_dunder_init(intake: Intake):
    intake.init()
    assert intake.note_in_intake is False
...
def test_set_inner_velocity(test_input, intake: Intake):
    ...
    intake.inner_motor.set_target_velocity.assert_called_with(
        test_input * constants.intake_inner_gear_ratio
    )
...
def test_set_outer_velocity(test_input, intake: Intake):
    ...
    intake.outer_motor_back.set_target_velocity.assert_called_with(
        test_input * constants.intake_outer_gear_ratio
    )
    intake.outer_motor_front.set_target_velocity.assert_called_with(
        test_input * constants.intake_outer_gear_ratio
    )
...
def test_roll_in(intake: Intake):
    intake.roll_in()
    intake.outer_motor_back.set_target_velocity.assert_called_with(
        config.intake_outer_speed * constants.intake_outer_gear_ratio
    )
    intake.outer_motor_front.set_target_velocity.assert_called_with(
        config.intake_outer_speed * constants.intake_outer_gear_ratio
    )
    intake.inner_motor.set_target_velocity.assert_called_with(
        config.intake_inner_speed * constants.intake_inner_gear_ratio
    )
...
def test_idle_in(intake: Intake):
    ...
    intake.outer_motor_back.set_target_velocity.assert_called_with(
        config.intake_outer_idle_speed * constants.intake_outer_gear_ratio
    )
    intake.outer_motor_front.set_target_velocity.assert_called_with(
        config.intake_outer_idle_speed * constants.intake_outer_gear_ratio
    )
