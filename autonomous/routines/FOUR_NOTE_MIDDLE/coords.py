...
get_second_note = (
    ...
    Field.POI.Coordinates.Notes.Wing.kCenter.withOffset(Translation2d(-constants.drivetrain_length/3, 0)).withRotation(0)
)

get_third_note = (
    get_second_note[2].withRotation(-90),
    [],
    Field.POI.Coordinates.Notes.Wing.kLeft.withOffset(Translation2d(-1*constants.drivetrain_length/3, 0)).withRotation(-90)
)

go_to_midline = (
    Field.POI.Coordinates.Notes.Wing.kLeft.withOffset(Translation2d(-2*constants.drivetrain_length/3, 0)),
    [],
    Field.POI.Coordinates.Notes.MidLine.kFarLeft.withOffset(Translation2d((-2 * constants.drivetrain_length / 3) + 0.2, 0))
)

shoot_last_note = (
    go_to_midline[2],
    [],
    Field.POI.Coordinates.Notes.Wing.kLeft.withOffset(Translation2d(2, 0))
)