...
compbot: DigitalInput = DigitalInput(0) # if true, we are using the practice bot (we will put a jumper on the DIO port)
...
front_left_encoder_zeroed_pos = 0.678 if compbot.get() else 0.0

front_right_move_id = 6
front_right_turn_id = 7
front_right_encoder_port = AnalogEncoder(2)
front_right_encoder_zeroed_pos = 0.503 if compbot.get() else 0.0

back_left_move_id = 2
back_left_turn_id = 3
back_left_encoder_port = AnalogEncoder(1)
back_left_encoder_zeroed_pos = 0.964 if compbot.get() else 0.0

back_right_move_id = 8
back_right_turn_id = 9
back_right_encoder_port = AnalogEncoder(3)
back_right_encoder_zeroed_pos = 0.260 if compbot.get() else 0.0

driver_centric: bool = True
drivetrain_reversed: bool = False

#pathing
post_avoidance_distance = 10 * inches_to_meters
